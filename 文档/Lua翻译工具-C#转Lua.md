...
|            |          |          |                    |
dfadsfasdfasdfasfasdfdasfasdadsfsd
## 一、文档摘要
...
1. 首先利用Mono Cecil库分析DLL中的类、字段、方法签名，然后将其翻译成对应的Lua类型结构;
2. 接着通过ILSpy工具分析IL指令集，生成由语句表达式组成的抽象结构树，并翻译成对应的Lua方法体;
3. 最后把Lua类型结构与Lua方法体合并成完整的Lua代码文件;
...
1. GWT：全称是Google Web Toolkit，这个工具能把Java语言翻译成JavaScript，降低前端项目开发和维护难度
2. Bridge.NET：这是一个开源项目，它能够把C#语言翻译成JavaScript
3. CoffeeScript：这是一个把自定义的类似ruby的语言翻译成JavaScript
...
​	本节详细介绍翻译流程，TKLua翻译流程，并没有去分析C#源代码，而是分析程序集DLL。在C#源代码经过编译得到DLL程序集之后，流程上经过两步对程序集DLL进行分析：

​	第一步：类型结构翻译，通过Mono Cecil分析DLL中的包含的所有类，以及类中定义的字段和方法定义，收集到这些信息，于是可以生成Lua的对应的类型和结构及方法定义。注意的是，此时所得到的方法定义只包含方法签名，无法得到方法体。

​	第二步：方法体翻译，利用ILSpy将方法体里面的IL指令转换成抽象语法数(AST)结构，翻译工具将AST转换成Lua语句和表达式，形成Lua方法体。

​	最后把第一步输出的Lua类型结构与第二步输出的Lua方法体，合成完整的Lua文件，从而实现了C#到Lua的翻译过程。
...
**STEP 1 :** 图最左边是C#源代码，定义了int x=32,int y=18,return x+y三条语句。
...
**STEP 4 :** 最后分析抽象结构树AST，并查找对应的符号表，最后翻译生成Lua对应的语句
...
<<<<<<< HEAD
1. C#中调用基类函数base.Awake()，翻译之后变成Lua中Awake(self)
2. 泛型函数binding<...>()，翻译之后泛型参数变成方法参数binding(...)
3. C#迭代器，翻译之后对应Lua迭代器
4. override、虚函数
5. 多次调用seats.Add，翻译之后变成缓冲函数F_Add
...
​	在Lua中赋值是没有返回值，故无法对变量进行连续赋值，TKLua采用的方案是拆解表达式，如下图，把y=x=foo()拆解成两次独立赋值，利用临时变量csl_0作为中间存储：csl_0=foo();x=csl_0;y=csl_0;
...
* switch中case判定，用if...else...模拟判定
* switch中break跳转，用repeat...break模拟跳转
...
​	由于在Lua中，没有continue语句，所以翻译过程中，需要用其它语句来模拟。在一个循环块中可能同时存在任意数量的continue和break，如下图，TKLua翻译采用的是用内嵌repeat循环和break模拟：

1. 增加内嵌repeat循环层
2. 原C#中一个continue，翻译成lua中一个break，仅跳出内层循环
3. 原C#中一个break，翻译成lua中两个break，跳出两层循环
